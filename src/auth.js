export const auth = async () => {
  ...
  // 1) get config
  const { tenantDomain, qlikWebIntegrationId, appId, sheetId, theme, currentLoginType, loginTypes } = await fetch("config").then((resp) => resp.json());
  const config = { tenantDomain, qlikWebIntegrationId, appId, sheetId, theme,  currentLoginType, loginTypes };
  // 2) get logged in
